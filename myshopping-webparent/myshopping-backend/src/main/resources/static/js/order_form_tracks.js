...
$(document).ready(function() {
	...
	$("#trackList").on("click", ".linkRemoveTrack", function(e) {
		e.preventDefault();
		deleteTrack($(this));//xóa orderTrack
		updateTrackCountNumbers();//cập nhật lại toàn bộ count
	});
	...
	$("#trackList").on("change", ".dropDownStatus", function(e) {
		...
		defaultNote = selectedOption.attr("defaultDescription");//lấy defaultDescription của enum đang chọn
		$("#trackNote" + rowNumber).text(defaultNote);//gán defaultDescription vào thẻ trackNote
	});	
});
...
//tạo ra orderTrack và thêm vào trackList
