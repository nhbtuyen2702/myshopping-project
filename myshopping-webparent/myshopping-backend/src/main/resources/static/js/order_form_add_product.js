...
$(document).ready(function() {
	...
	$("#products").on("click", "#linkAddProduct", function(e) {
		...
		$("#addProductModal").on("shown.bs.modal", function() {//sau khi modal addProductModal đã được hiển thị thì nó sẽ vào .on("shown.bs.modal"
			$(this).find("iframe").attr("src", url);//tìm đến thẻ iframe bên trong và thay đổi giá trị của thuộc tính src
		});
		
		$("#addProductModal").modal();//hiển thị modal
	})
});

function addProduct(productId, productName) {
	getShippingCost(productId);//tính phí ship
}

function getShippingCost(productId) {//gọi xuống controller để tính phí ship
	...
	$.ajax({
		...
		getProductInfo(productId, shippingCost);//sau khi tính phí ship sẽ gọi xuống controller để lấy thông tin của product dựa vào productId
	}).fail(function(err) {
...
//gọi xuống controller để lấy product dựa vào productId
function getProductInfo(productId, shippingCost) {
	...
	$.get(requestURL, function(productJson) {
		...
		htmlCode = generateProductCode(productId, productName, mainImagePath, productCost, productPrice, shippingCost);//lấy các thông tin của product để tạo html tương ứng
		$("#productList").append(htmlCode);//thêm thẻ mới vào bên trong productList
		
		updateOrderAmounts();//cập nhật lại toàn bộ giá trị trong tab overview
...
//copy div trong order_form_products và thay đổi các giá trị cần thiết
...
//kiểm tra product này đã tồn tại trong list orderDetails chưa, nếu chưa thì cho phép thêm, nếu có thì hiện thông báo đã tồn tại
