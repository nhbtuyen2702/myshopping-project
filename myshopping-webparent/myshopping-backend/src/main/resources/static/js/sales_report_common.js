...
function setupButtonEventHandlers(reportType, callbackFunction) {
	$(".button-sales-by" + reportType).on("click", function() {
		...
		if (period) {
			...
			$("#divCustomDateRange" + reportType).removeClass("d-none");//khi chọn Custom Date Range thì period là sẽ undefined vì ko khai báo, bỏ class d-none để nó hiện cho chọn start date và end date 		
		}		
	});
	
	initCustomDateRange(reportType);
	
	//click vào View khi đã chọn start date và end date
	$("#buttonViewReportByDateRange" + reportType).on("click", function(e) {
		validateDateRange(reportType, callbackFunction);
	});	
}

//kiểm tra nhập start date và end date có hợp lệ hay ko
//chỉ cho phép nhận start date và end date trong khoảng [7;30]
...
//tính số ngày dựa trên start date và end date
...
//gán giá trị start date và end date cho tất cả các loại sales report
...
//gán các giá trị cho các dữ liệu bên dưới biểu đồ
...
function formatChartData(data, columnIndex1, columnIndex2) {
	var formatter = new google.visualization.NumberFormat({
		prefix: prefixCurrencySymbol,//ký tự đặt trước giá trị số. 
		suffix: suffixCurrencySymbol,//ký tự đặt sau giá trị số.
		decimalSymbol: decimalPointType,//ký tự dùng để phân cách phần nguyên và phần thập phân.
		groupingSymbol: thousandsPointType,//ký tự dùng để phân cách các số hàng nghìn.
		fractionDigits: decimalDigits//số chữ số thập phân được hiển thị.
	});
