...
import com.shoppingcart.admin.order.OrderRepository;
...
public class MasterOrderReportService extends AbstractReportService {
	...
	private void calculateSalesForReportData(List<Order> listOrders, List<ReportItem> listReportItems) {
		for (Order order : listOrders) {
			...
			int itemIndex = listReportItems.indexOf(reportItem);//tìm orderDate trong listReportItems, nếu có identifier trong reportItem trùng với orderDate thì gán các giá trị còn lại trong reportItem 
	...
	private List<ReportItem> createReportData(Date startTime, Date endTime, ReportType reportType) {
		...
		do {
			if (reportType.equals(ReportType.DAY)) {
				startDate.add(Calendar.DAY_OF_MONTH, 1);//lấy ngày hiện tại công thêm số ngày cần tính(vì dùng ngày nên DAY_OF_MONTH) 
			...
			listReportItems.add(new ReportItem(dateString));//nếu chọn 28 days thì listReportItems sẽ có 28 phần tử
