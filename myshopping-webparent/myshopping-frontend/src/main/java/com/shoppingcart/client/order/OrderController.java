...
public class OrderController {
	...
	public String viewOrderDetails(Model model,
		...
		//gán giá trị của reviewedByCustomer và customerCanReview cho product hiện tại
		//trong trang order_details_modal nếu: 
		//reviewedByCustomer = false và customerCanReview = true -->customer có thể write review cho product hiện tại
		//reviewedByCustomer = true -->customer có thể xem review cho product hiện tại
	...
	private void setProductReviewableStatus(Customer customer, Order order) {
		...
		while(iterator.hasNext()) {
			...
			boolean didCustomerReviewProduct = reviewService.didCustomerReviewProduct(customer, productId);//didCustomerReviewProduct = true -->customer đã từng review
			product.setReviewedByCustomer(didCustomerReviewProduct);
			
			if (!didCustomerReviewProduct) {
				boolean canCustomerReviewProduct = reviewService.canCustomerReviewProduct(customer, productId);//canCustomerReviewProduct = true -->customer có thể xem review
